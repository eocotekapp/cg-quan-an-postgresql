...
function cleanLegacyImageValue(v){
  const str = String(v || "").trim();
  if(!str) return "";
  // Giữ tương thích ảnh cũ dạng URL hoặc endpoint ảnh đã lưu.
  if(str.startsWith("/api/menu?image=") || str.startsWith("api/menu?image=")) return str;
...
module.exports = async function handler(req,res){
  try{
    ...
    if(req.method==="POST"){
      ...
      const incomingImage = String(b.imageUrl || b.imageData || "").trim();
      const dataImage = parseDataImage(incomingImage);
      let finalImageUrl = dataImage ? imageUrlFor(id) : cleanLegacyImageValue(incomingImage);
