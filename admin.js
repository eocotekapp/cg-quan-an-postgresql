...
/* ===== BIỆN PHÁP BẢO VỆ CHỐNG SỰ CỐ: KHỬ PHÍ SHIP & TAB CÀI ĐẶT TOÀN CỤC ===== */
(function() {
  // 1. Ép tất cả các công thức tính toán liên quan đến phí ship trong tương lai luôn bằng 0
  Object.defineProperty(window, 'shippingFee', { get: () => 0, set: () => {} });
  Object.defineProperty(window, 'freeShipFrom', { get: () => 999999999, set: () => {} });

  // 2. Định chặn lỗi khi logic cố đọc thuộc tính từ các ô input phí ship đã bị xóa ở HTML
  const originalQuerySelector = document.querySelector;
  document.querySelector = function(selector) {
    const el = originalQuerySelector.apply(this, arguments);
    if (!el && (selector.includes('shipping') || selector.includes('settings') || selector.includes('ship'))) {
      // Tạo một phần tử ảo để tránh crash lỗi ".value" hoặc ".innerHTML"
      return document.createElement('div');
    }
    return el;
  };

  // 3. Chặn và ép giá trị shipping_fee trong mọi gói dữ liệu đơn hàng trả về từ API về 0
  const originalFetch = window.fetch;
  window.fetch = async function(...args) {
    const res = await originalFetch(...args);
    if (args[0] && (args[0].includes('/api/orders') || args[0].includes('/api/settings'))) {
      try {
        const clone = res.clone();
        const data = await clone.json();
        if (data && typeof data === 'object') {
          if (Array.isArray(data)) {
            data.forEach(o => { if(o) o.shipping_fee = 0; o.shippingfee = 0; });
          } else if (data.orders && Array.isArray(data.orders)) {
            data.orders.forEach(o => { if(o) o.shipping_fee = 0; o.shippingfee = 0; });
          } else if (data.settings) {
            data.settings.shippingFee = 0;
            data.settings.freeShipFrom = 999999999;
          }
        }
