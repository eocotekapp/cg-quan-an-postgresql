...
function deleteUploadedImage(imageUrl){
  try{
    ...
    console.warn("Lỗi xoá ảnh:", e.message);
  }
  return false;
}
...
module.exports = async function handler(req,res){
  try{
    ...
    if(req.method==="POST"){
      ...
      const b=req.body||{};
      let oldImageUrlForReplace="";
      try{
        const oldResult=await query("SELECT image_url FROM menu WHERE id=$1",[cleanString(b.id||slug(b.name),120)]);
        oldImageUrlForReplace=rows(oldResult)?.[0]?.image_url||"";
      }catch(e){}
      ...
      if(oldImageUrlForReplace && b.imageUrl && oldImageUrlForReplace!==b.imageUrl){
        deleteUploadedImage(oldImageUrlForReplace);
      }
      return send(res,200,{ok:true,id});
    }
    if(req.method==="DELETE"){
      if(!requireAdmin(req,res)) return;
      let oldImageForDelete="";
      try{
        const oldResult=await query("SELECT image_url FROM menu WHERE id=$1",[req.query.id]);
        oldImageForDelete=rows(oldResult)?.[0]?.image_url||"";
      }catch(e){}
      await query("DELETE FROM menu WHERE id=$1",[req.query.id]);
      if(oldImageForDelete){
        deleteUploadedImage(oldImageForDelete);
      }
